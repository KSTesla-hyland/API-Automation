...
scenarios("../Features/GeographyController_Adminapi.feature")
...
@given(parsers.parse('User creates new endpoint from newly created Geography using "{endpointname}" api'))
def store_Id_and_endpoint_of_new_Geography(api_request_context_auth, api_respone_context, endpointname,
                                                test_data_store):
    ...
    Geography_update = miscUtils.get_id_by_name_from_response(response, "Test","State")
    test_data_store["Geography_id"] = Geography_update

@given(parsers.parse('User hits the put api for Geography of apiadmin'))
def edit_the_Geography(api_request_context_auth, api_respone_context, test_data_store):
    geography_id = test_data_store.get("Geography_id")
    endpoint = f"{geography_id}"
    uuid_only = endpoint.split("/")[-1]
    final_data = miscUtils.readFileContentAsString("TestData/POST_Data/EditGeography.json")
...
@given(parsers.parse('User hits the delete api for Geography of apiadmin'))
def delete_the_Geography(api_request_context_auth, api_respone_context, test_data_store):
    geography_id = test_data_store.get("Geography_id")
    endpoint = f"{geography_id}"
