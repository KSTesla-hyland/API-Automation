...
scenarios("../Features/ModalityController_Adminapi.feature")
...
@given(parsers.parse('User creates new endpoint from newly created Modality using "{endpointname}" api'))
def store_Id_and_endpoint_of_new_Modality(api_request_context_auth, api_respone_context, endpointname,
                                                test_data_store):
    ...
    Modality_update = miscUtils.get_code_from_response(response, "API","Modality")
    test_data_store["Modality_id"] = Modality_update

@given(parsers.parse('User hits the put api for Modality of apiadmin'))
def edit_the_Modality(api_request_context_auth, api_respone_context, test_data_store):
    modality_id = test_data_store.get("Modality_id")
    endpoint = f"{modality_id}"
    uuid_only = endpoint.split("/")[-1]
    uuid_decoded = base64.b64decode(uuid_only).decode("utf-8")
    final_data = miscUtils.readFileContentAsString("TestData/POST_Data/EditModality.json")
...
@given(parsers.parse('User hits the delete api for Modality of apiadmin'))
def delete_the_Modality(api_request_context_auth, api_respone_context, test_data_store):
    modality_id = test_data_store.get("Modality_id")
    endpoint = f"{modality_id}"
