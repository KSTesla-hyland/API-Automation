...
scenarios("../Features/GroupController_Adminapi.feature")
...
@given(parsers.parse('User creates new endpoint from newly created Group using "{endpointname}" api'))
def store_Id_and_endpoint_of_new_Group(api_request_context_auth, api_respone_context, endpointname,
                                                test_data_store):
    ...
    Group_update = miscUtils.get_groupid_by_name_from_response(response, "Test","Group")
    test_data_store["Group_id"] = Group_update

@given(parsers.parse('User hits the put api for Group of apiadmin'))
def edit_the_Group(api_request_context_auth, api_respone_context, test_data_store):
    group_id = test_data_store.get("Group_id")
    endpoint = f"{group_id}"
    uuid_only = endpoint.split("/")[-1]
    final_data = miscUtils.readFileContentAsString("TestData/POST_Data/EditGroup.json")
    data = json.loads(final_data)
    data["groupId"] = uuid_only
...
@given(parsers.parse('User hits the delete api for Group of apiadmin'))
def delete_the_Group(api_request_context_auth, api_respone_context, test_data_store):
    group_id = test_data_store.get("Group_id")
    endpoint = f"{group_id}"
