...
scenarios("../Features/SubspecialityController_Adminapi.feature")
...
@given(parsers.parse('User creates new endpoint from newly created subspecialty using "{endpointname}" api'))
def store_Id_and_endpoint_of_new_subspecialty(api_request_context_auth, api_respone_context, endpointname,
                                                test_data_store):
    ...
    Subspecialty_update = miscUtils.get_id_by_name_from_response(response, "API","Subspecialty")
    test_data_store["Subspecialty_id"] = Subspecialty_update
    print(Subspecialty_update)

@given(parsers.parse('User hits the put api for subspecialty of apiadmin'))
def edit_the_subspecialty(api_request_context_auth, api_respone_context, test_data_store):
    subspecialty_id = test_data_store.get("Subspecialty_id")
    endpoint = f"{subspecialty_id}"
    uuid_only = endpoint.split("/")[-1]
    final_data = miscUtils.readFileContentAsString("TestData/POST_Data/EditSubspecialty.json")
...
@given(parsers.parse('User hits the delete api for subspecialty of apiadmin'))
def delete_the_subspecialty(api_request_context_auth, api_respone_context, test_data_store):
    subspecialty_id = test_data_store.get("Subspecialty_id")
    endpoint = f"{subspecialty_id}"
