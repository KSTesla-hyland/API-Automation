...
scenarios("../Features/LauncherFilterController_Adminapi.feature")
...
@given(parsers.parse('User creates new endpoint from newly created Launcher Filter using "{endpointname}" api'))
def store_Id_and_endpoint_of_new_Launcher_Filter(api_request_context_auth, api_respone_context, endpointname,
                                                test_data_store):
    ...
    LauncherFilter_update = miscUtils.get_id_by_name_from_response(response, "API","LauncherFilter")
    test_data_store["LauncherFilter_id"] = LauncherFilter_update

@given(parsers.parse('User hits the put api for Launcher Filter of apiadmin'))
def edit_the_Launcher_Filter(api_request_context_auth, api_respone_context, test_data_store):
    launcherFilter_id = test_data_store.get("LauncherFilter_id")
    endpoint = f"{launcherFilter_id}"
    uuid_only = endpoint.split("/")[-1]
    final_data = miscUtils.readFileContentAsString("TestData/POST_Data/EditLauncherFilter.json")
...
@given(parsers.parse('User hits the delete api for Launcher Filter of apiadmin'))
def delete_the_Launcher_Filter(api_request_context_auth, api_respone_context, test_data_store):
    launcherFilter_id = test_data_store.get("LauncherFilter_id")
    endpoint = f"{launcherFilter_id}"
